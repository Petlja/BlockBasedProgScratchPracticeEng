...
.. topic:: Речник и визуелна граматика језика Скреч
 ...
 .. hlist::
    ...
    * |motion_blocks| Кретање,
    * |looks_blocks| Изглед, 
    * |sound_blocks| Звук, 
    * |events_blocks| Догађаји,
    * |control_blocks| Управљање, 
    * |sensing_blocks| Осећаји,
    * |operator_blocks| Оператори,
    * |variables_blocks| Променљиве, 
    * |my_blocks| Моји блокови и 
 ...
 **Самостални блокови**
 ...
 Блокове који одговарају корацима обраде зваћемо надаље **стек блокови**. Сваки стек блок одговара једној акцији, односно одговара на питање ШТА треба урадити. На пример, блок |move_steps| наређује лику да се помери задати број корака у смеру у коме је тренутно постављен. Облик овог блока подсећа на Lego коцку, а цела скрипта на коцке склопљене у стек. 

 Задати број корака уписује се у бели кружић - **улазно поље** блока. 

 У блоку |say_sec| једно улазно поље има облик заобљеног правоугаоника, а друго круга. Правоугаони улази прихватају и текстуалне и бројне, а кружни само бројне вредности.

 Блок |glide_xy| има чак три улаза.

 Неки блокови имају шестоугаона улазна поља, на пример управљачки блок |wait_until|. Они прихватају логичке вредности (тачно или нетачно).

 У групи *Управљање* постоје блокови С-облика, које називамо **С-блокови**. Слот унутар С-блока је специјална врста улаза који прима скрипту као улаз. Блок  |repeat| има 2 улаза: један за број и један за скрипту. 
...
Сценарио овог пројекта је сасвим једноставан. Лик наредбама |say_sec| задаје кориснику прва три захтева, а наредбом |say| саопштава резултат.

Користићемо лик девојчице Avery из библиотеке ликова који има два костима (Кликни на картицу *Костими* да их видиш) и између свака два захтева задати промену костима наредбом |next_costume|.
...
Наредба |say| последњи текст оставља на екрану све док не прекинемо извршавање програма кликом на знак *стоп* - црвени осмоугао изнад горњег десног угла позорнице.
...
.. reveal:: упутство
     ...
     **Упутство:**
     ...
     Девојчица је тражила да се замисли троцифрен број са истим цифрама. Означимо цифру која се понавља са х. Тада је замишљен број једнак 111х, а збир цифара је 3х. Резултат операције дељења броја збиром његових цифара једнак је 111/3=37, независно од броја који је замишљен.

.......

Example 3 - Project „Рачунање”
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 **Генерисање случајних бројева** 
 ...
 У групи блокова *Оператори* налази се наредба |pick_random|. Генерисати случајан број значи на случајан начин одабрати број из неког интервала. Најмањи и највећи број тог интервала задаје се у пољима овог репортера. Значи, ако су то бројеви 1 и 10 биће изабран ма који број из интервала [1,10], а да су, на пример, то -100 и 100, био би генерисан неки број из интервала [-100,100]. 
...
|5| Направи променљиву *zbir* и као почетну вредност јој додели вредност израза x+x+x. Како блок за сабирање има само 2 улазна поља, Уложићемо један блок за сабирање у улазно поље другог блока за сабирање. (Разуме се, исто смо могли постићи и једним блоком за множење јер је х+х+х=3*х)
...
Example 4 - Project „Ефекти на лик”
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. level:: 1

Следећи пројекат илуструје како наредба |change_effect| из групе *Изглед* може да утиче на приказ ликова. Такође показује и како догађај *клик на лик* може да покрене извршавање скрипте.

**Сценарио пројекта**

На позорници се налази 7 примерака истог лика. Кликом на неки од њих лик се мења коришћењем једног од седам графичких ефеката. То омогућава наредба |change_effect|. Лик такође изговара о којем се ефекту ради. 
...
|5| Промени ефекат у наредби |change_effect| на *рибље око*, а у наредби |say_sec| преправи текст у *рибље око*.
...
|9| Зато треба додати скрипту позорници која ће по клику на зелену заставицу поништавати промене наредбом |clear_effects|.
...
Example 5 - Project „Ефекти на позадину”
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. level:: 1

За разлику од блокова из групе *Кретање*, блокови из групе *Изглед* могу се применити и на позорницу. Све скрипте у овом пројекту придружене су позорници. Сврха пројекта је да илуструје како наредба |change_effect| може да утиче на изглед позадине и да ти да идеје како би сам могао да је користиш у својим пројектима. Да би се све вратило у нормалу, догађају *клик на зелену заставицу* придружено је брисање свих графичких ефеката. 
...
.. mchoice:: блокови4
   ...
   Којој врсти припадају блокови чувају вредности и не могу да стоје самостално у скрипти, већ се умећу у улазна поља других блокова. 
...
.. dragndrop:: групе_наредби
    ...
    :match_1: почетни|||дефинишу ШТА треба урадити
    :match_2: репортери|||задају КАДА треба покренути скрипту
    :match_3: С-блокови|||примају скрипту као улаз
    :match_4: стек блокови|||чувају вредности
...
.. mchoice:: аi01
   ...
   Да ли изрази |q3_8a| и |q3_8b| имају исту вредност?    
...
.. mchoice:: аi02
   ...
   Који од израза  |q3_9a| и |q3_9b| има негативну вредност?
...
.. fillintheblank:: ai03
      ...
      Колика је вредност израза |q3_10| |blank|
...
.. mchoice:: ai04
   ...
   Колика је вредност израза  |q3_11|?
...
.. mchoice:: ai05
   ...
   Колика је вредност израза |q3_12|?
...
.. reveal:: вежба 3_1
   ...
   **Упутство:**
   ...
   Лику треба придружити следеће скрипте: |ex3_1| 
...
.. reveal:: вежба 3_2
   ...
   **Упутство:**
   ...
   Лику мачак треба придружити следеће скрипте: |ex3_2| 

.. |ex3_2| image:: ../_images/3/ex3_2.png

.......

Exercise 3 - Промена боје позадине
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. reveal:: вежба 3_3
   ...
   **Упутство:**
   ...
   Позорници треба придружити следеће скрипте: |ex3_3| 

.. |ex3_3| image:: ../_images/3/ex3_3.png
  
.......

Exercise 4 - Кодирање аритметичког израза са заградама (1)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Exercise 5 - Кодирање аритметичког израза са заградама (2)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:Питање:	
   Ученик воли да све ради по правилима и зато жели да сваком програму дода и наредбу за заустављање програма. Тако је и на крај скрипте за математичара поставио наредбу |stop|. Међутим, текст последње наредбе |say| уопште се није видео. 
...
**Наредбе**: |events_blocks| - |clicked_sprite|, |clicked_stage|; |control_blocks| - |wait_sec|, |stop|; 

|looks_blocks| - |think_sec|, |next_costume|, |change_effect|, |clear_effects|, |set_size|, |change_size|; |operator_blocks| - |plus|, |minus|, |multiply|, |divide|, |pick_random|; |variables_blocks| - |*| |set_var|.
