...
.. infonote::
  ...
  **Како се креирају променљиве?**
  ...
  Монитор може да приказује вредности променљивих у више различитих формата приказаних на следећој слици.

  .. image:: ../_images/7/fig7_3.png
         :width: 240px   
         :align: center

  (A) мали приказ вредности са именом променљиве,

  (B) велики приказ вредности без припадајућег имена, 

  (C) са клизачем који омогућава промену вредности променљиве.
...
Услов у наредби |wait_until| је сложен од два услова: мачак 1 додирује миша или мачак 2 додирује миша. 
...
*Време* на жељено трајање игре, на пример 120 секунди. Затим ћемо у циклусу који се понавља 120 пута извршавати наредбу |wait_sec| и смањивати вредност променљиве за 1.
По изласку из циклуса поставићемо наредбу |stop|. И ову скрипту придружићемо мишу.
...
Example 4 - Project „Мали математичар”
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Операција |join| је једна од операција која се може извршити над подацима типа стринг (ниска знакова). 
...
.. reveal:: објашњење
   ...
   У поглављу *Циклуси* направили смо класификацију блокова који се користе у Скречу и указали да се тзв. *Функцијски блокови* умећу у тзв. *Командне блокове*. 
   ...
   Треба да добијемо |expr1|

   .. |expr1| image:: ../_images/7/fig7_21.png
 
   1. Како да добијемо  |expr2| и |expr3| ?

   .. |expr2| image:: ../_images/7/fig7_26.png

   .. |expr3| image:: ../_images/7/fig7_27.png

   а) Узмемо блок  |expr4| из групе блокова *Осећаји*. 

   .. |expr4| image:: ../_images/7/fig7_28.png

   б) Из листе ликова коју добијемо кликом на троуглић уз лик *Математичар* изаберемо *Број1* (односно *Број2*)
   в) Из листе променљивих придружених овом лику коју добијемо када кликнемо на троуглић уз променљиву *место х* изаберемо *број костима*
   ...
   2. Како да добијемо  |expr6|

   .. |expr6| image:: ../_images/7/fig7_30.png

   а) Узмемо блок |multiply|  из групе блокова *Операције*

   б) Поставимо први аргумент који смо добили по опису 1 у прво поље  |expr7|

   .. |expr7| image:: ../_images/7/fig7_31.png

   в) Поставимо други аргумент који смо добили по опису 1 у друго поље 

   3. Како да добијемо |expr8|

   .. |expr8| image:: ../_images/7/fig7_32.png

   а) Узмемо блок |join|  из групе блокова *Операције*

   б) Упишемо текст "Одговор је " у прво поље |expr9| 

   .. |expr9| image:: ../_images/7/fig7_33.png

   в) Поставимо аргумент који смо формирали по опису 2 у друго поље

   4. Коначно, овако формиран аргумент поставимо у поље блока |think|
...
Да бисмо избегли проблеме код дељења, ако се приликом генерисања операције открије да је то дељење,  разгласићемо поруку *подели*, а у свим осталим случајевима радићемо на начин као што је то било решено у претходним верзијама пројекта.
...
Оно што се јавља као нови проблем у оваквом решењу задатка је то да ће дељеник вероватно бити двоцифрен број, а може се завршавати и цифром 0 која у претходним пројектима није била укључена у костиме за *Број1*. 
Овај проблем решавамо тако што уводимо још један лик, умножак лика Број1 који смо назвали (цифра) *Десетица*, а лику *Број1* додајемо нулу као 10. костим. Подразумева се да се лик *Дестица* не види, појављује се једино када *Знак* пошаље поруку дељење, а претходно израчунати дељеник је двоцифрен број (што детектује променљива број цифара). 
Тада се *Десетица* (цифра десетица дељеника) појављује лево од лика *Број1*, који добија вредност цифре јединица дељеника. 

За издвајање цифара дељеника користимо операцију |letter| која издваја знак са наведеним редним бројем из стринга. 
...
.. mchoice:: ai16
   ...
   :answer_c:
   :correct: a
   :feedback_a: с је помоћна променљива.
   :feedback_b: а је помоћна променљива, вредности размењују b и С.
   :feedback_c: Ако је b=0, дељење није дефинисано.
...
.. mchoice:: for10a
   ...
   :correct: c
   :feedback_a:  
   :feedback_b: Услов гранања је испуњен, па се полазна вредност променљиве увећава.
   :feedback_c: 
   :feedback_d: 
...
.. mchoice:: if08a
   ...
   :answer_d: a=4
   :correct: b, d
   :feedback_a: Није тачно да је 5<5, услов гранања није испуњен. 
   :feedback_b: 0<5.
   :feedback_c: Није тачно да је 10<5, услов гранања није испуњен.
   :feedback_d: 4<5.
...
.. mchoice:: if09a
   ...
   :correct: a
   :feedback_a: 5<10.  
   :feedback_b: Пошто је 0>-1, услов гранања није испуњен. 
   :feedback_c: Пошто је 10>5, услов гранања није испуњен.
   :feedback_d: Пошто је 4=4, услов гранања није испуњен.
...
.. mchoice:: if13b
   ...
   :feedback_a:  
   :feedback_b: 
   :feedback_c: Услов гранања није испуњен, па се полазна вредност променљиве не мења.
   :feedback_d: 
...
**New concepts**: променљива - проста, сложена, локална, глобална.

**Strach commands**: |variables_blocks| - |set_var|, |change_var|, |show_var| , |hide_var|; |sensing_blocks| - |touching_color|, |parameter_of|; |operator_blocks| - |or|, |mod|, |join|, |function|


|project| Create a projects
---------------------------
Project 1 - „Калкулатор”
~~~~~~~~~~~~~~~~~~~~~~~~~
...
Project 2 - Програм на основу блок дијаграма
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
